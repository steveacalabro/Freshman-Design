...
Servo servoTilt, servoPan, setServo;

//This is a character that will hold data from the Serial port.
char serialChar=0;

void setup(){
  servoTilt.attach(9);  //The Tilt servo is attached to pin 2.
  servoPan.attach(8);   //The Pan servo is attached to pin 3.
  setServo.attach(7);
  setServo.write(110);
  servoTilt.write(136);  //Initially put the servos both
...
void loop(){
  ...
  else if(serialChar == 2){ //Check to see if the initial serial character was the servo ID for the pan servo.
    while(Serial.available() <= 0);  //Wait for the second command byte from the serial port.
    pos = servoTilt.read();
    Serial.println(pos);  //Set the pan servo position to the value of the second command byte received from the serial port.
  }
  else if(serialChar == 3){ //Check to see if the initial serial character was the servo ID for the pan servo.
    while(Serial.available() <= 0);  //Wait for the second command byte from the serial port.
    pos = servoPan.read();
    Serial.println(pos);   //Set the pan servo position to the value of the second command byte received from the serial port.
  }
